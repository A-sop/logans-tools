import { previewKindForPath } from '@/lib/atlas-ops/document-intake/review-path-guard';
import { listUnprocessedInboxFiles, processUnprocessedInboxBatch } from '@/lib/atlas-ops/document-intake/process-inbox-batch';
import { DIL_CLASSIFY_BATCH_MAX, DIL_REVIEW_UI_BATCH, clampClassifyBatch, clampReviewBatch } from '@/lib/atlas-ops/document-intake/review-config';
import {
  mergeFilingOverrides,
  parseReviewNotes,
  resolveEffectiveFiling,
} from '@/lib/atlas-ops/document-intake/review-filing';
import type {
  ProcessBatchResult,
  ...
  ReviewQueueResponse,
  ReviewStats,
} from '@/lib/atlas-ops/document-intake/review-types';
import { recordFilingLearning, recordNotedIntentLearning, applyLearnedFilingToProposal, parseFilingMetadata } from '@/lib/atlas-ops/document-intake/registry-filing-learn';
import { getRegistryPaths } from '@/lib/atlas-ops/document-intake/registry-config';
import { loadPilotV3IndexSync, type PilotV3Extraction } from '@/lib/atlas-ops/document-intake/pilot-v3-store';
import { DocumentRegistryIndex, type RegistryReviewRow } from '@/lib/atlas-ops/document-intake/registry-sqlite';
...
function rowToItem(
  ...
  if (!v3?.proposal && weakRegistryRename(row, registryProposedBasename)) {
    ...
    warnings.push('v3 pilot proposal matches registry — no upgrade needed.');
  } else if (v3 && !v3.proposal && (v3.chars ?? 0) > 0) {
    warnings.push(
      `v3 pilot OCR'd this file (${v3.chars} chars) but could not build a validated rename — see partial fields in v3 panel.`
    );
  }
  ...
  if (parsedNotes.inferredDelete) {
    warnings.push('Notes say delete — approve N (reject) and remove the file when ready.');
  }
  if (parsedNotes.inferredVentures && !merged.relativePathOverride) {
    warnings.push('Notes mention Ventures / Bridge Life — add path: in notes or set folder override before approve.');
  }
...
/** Server-side review adapter — swap this module to change backend without touching the UI. */
