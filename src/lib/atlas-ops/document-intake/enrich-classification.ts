...
import { detectPartnerCode, type PartnerCodeAlias } from '@/lib/atlas-ops/document-intake/partner-code-aliases';
import { buildTaxBelegNaming, proposedTaxRelativePath } from '@/lib/atlas-ops/document-intake/tax-naming';
import type {
...
function inferDocRole(text: string, fileName: string): DocRole {
  ...
  if (/\b(rechnung|invoice|beleg|quittung|kontoauszug|steuer|finanzamt|eür|ust)\b/i.test(haystack)) {
    return 'tax_evidence';
  }
 ...
 * derived/client versions stay linkable to the source form. PROPOSAL ONLY — the
...
function buildPartnerCodeNaming(
  ...
  const displayTitle = `${alias.issuerLegalName} — ${alias.docType} (${alias.code})`.slice(0, 120);
...
export function enrichClassification(
  ...
  if (partnerAlias) {
    // Recognised partner document code → propose a human-readable name carrying the
    ...
    warnings.push(
      `Recognised partner code ${partnerAlias.code} → ${partnerAlias.issuer} / ${partnerAlias.docType}.`
    );
    ...
    warnings.push('Keeping current filename — not confident enough to propose a rename.');
  } else {
