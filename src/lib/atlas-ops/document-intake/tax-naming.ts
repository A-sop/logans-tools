...
import { matchEuerLine } from '@/lib/atlas-ops/document-intake/euer-lines';
import { taxYearFolder } from '@/lib/atlas-ops/document-intake/registry-config';
import type { ClassificationResult, IntelligenceEnrichment } from '@/lib/atlas-ops/document-intake/types';
...
function isPrivateHat(text: string, organization: string): boolean {
  if (/\b(privat|private|pers[oö]nlich|krankenkasse|arzt|health)\b/i.test(text)) return true;
...
export function buildTaxBelegNaming(
  ...
  if (!lineMatch) {
    warnings.push('No EÜR Zeile matched; using intake naming fallback for basename.');
  ...
  if (zeileConfidence < 0.6) {
    warnings.push(`Low confidence for EÜR Zeile ${entry.zeile}.`);
  }
