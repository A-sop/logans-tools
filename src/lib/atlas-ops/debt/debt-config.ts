...
import type { DebtPaths, DebtSettings } from '@/lib/atlas-ops/debt/debt-types';
...
export function getDebtPaths(): DebtPaths {
  const root = readEnv('ATLAS_DEBT_ROOT') ?? DEFAULT_ROOT;
  const dbPath = readEnv('ATLAS_DEBT_DB_PATH') ?? path.join(root, 'atlas-debt.db');
  const seedDir = readEnv('ATLAS_DEBT_SEED_DIR') ?? DEFAULT_SEED_DIR;
