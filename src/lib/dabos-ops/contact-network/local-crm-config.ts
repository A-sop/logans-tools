...
import type { LocalCrmPaths } from '@/lib/dabos-ops/contact-network/local-crm-types';
...
export function getLocalCrmPaths(): LocalCrmPaths {
  const root =
    readEnv('LDW_CRM_ROOT') ??
    readEnv('DABOS_CRM_ROOT') ??
    readEnv('ATLAS_CRM_ROOT') ??
    DEFAULT_ROOT;
  const dbPath =
    readEnv('LDW_CRM_DB_PATH') ??
    readEnv('DABOS_CRM_DB_PATH') ??
    readEnv('ATLAS_CRM_DB_PATH') ??
    path.join(root, 'ldw-local-crm.db');
  const exportsDir =
    readEnv('LDW_CRM_EXPORTS_DIR') ??
    readEnv('DABOS_CRM_EXPORTS_DIR') ??
