...
} from '@/lib/dabos-ops/contact-network/local-crm-types';
...
export class LocalCrmIndex {
  ...
  public appendContactNotes(contactId: number, fragment: string): void {
    ...
    const notes = existing ? `${existing} Â· ${trimmed}` : trimmed;
    this.db
  ...
  /** Idempotent open-task seed: same contact + title while open â†’ reuse row (updates notes/due/priority). */
