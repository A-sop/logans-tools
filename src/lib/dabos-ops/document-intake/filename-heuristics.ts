...
import type { TextQuality } from '@/lib/dabos-ops/document-intake/types';

/** Scanner names like doc04343120260605085130.pdf embed YYYYMMDDâ€¦ */
...
export function extractScanToken(fileName: string): string {
  const base = fileName.replace(path.extname(fileName), '');
  // Scanner pattern: doc04343120260605085130 â†’ doc + 6-digit id, then YYYYMMDDâ€¦
...
export function isLowInformationPdfText(text: string): boolean {
  ...
  const letters = (trimmed.match(/[a-zA-ZÃ¤Ã¶Ã¼Ã„Ã–ÃœÃŸ]/g) ?? []).length;
 ...
 * text layer") that a length check misses â€” the COR1389 failure mode, where a
 * PDF's embedded text is long but is one junk glyph per real glyph.
 *
 * Discriminators (all local, deterministic):
 * - replacement/control-char ratio â€” a strong corruption signal;
 * - word-shaped-token ratio â€” fraction of whitespace tokens containing a 3+ letter
 *   run. Real prose is ~0.7â€“0.95; CID gibberish is near zero even when it maps
 *   into the Latin range. This is the key signal length/letterRatio both miss.
 * - letter ratio â€” secondary; kept deliberately loose so numeric-heavy but real
