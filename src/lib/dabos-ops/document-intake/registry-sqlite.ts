...
import { resolveEffectiveFiling } from '@/lib/dabos-ops/document-intake/review-filing';
import type { DocRole, NamingTrack, ProcessingState, ProposedBucket, RegistryClassification } from '@/lib/dabos-ops/document-intake/types';
...
export class DocumentRegistryIndex {
  ...
  public countReviewQueue(filter: ReviewQueueFilter = 'todo'): number {
    ...
    switch (filter) {
      case 'todo':
        // Riffed rows live in 'noted' ONLY â€” once Logan writes a note, the
  ...
  public getReviewDecisionStats(): {
    ...
    const todo = (
      this.db
        .prepare(
          // Noted rows are handled â€” excluded here exactly as in countReviewQueue('todo').
