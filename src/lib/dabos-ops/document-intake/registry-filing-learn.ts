...
import type { ResolvedFiling } from '@/lib/dabos-ops/document-intake/review-filing';
import type { DocumentRegistryIndex, FilingRuleStatus, RegistryFilingRuleRow } from '@/lib/dabos-ops/document-intake/registry-sqlite';
...
function ruleLabelForKind(kind: FilingRuleKind, matchValue: string, status: FilingRuleStatus): string {
  const prefix = status === 'draft' ? 'draft Â· ' : '';
  switch (kind) {
    case 'tax_anbieter':
