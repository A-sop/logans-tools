import type { ReviewApproval, ReviewStats } from '@/lib/dabos-ops/document-intake/review-types';
...
function motivationalMessage(input: {
  ...
  if (input.remaining === 0) return 'Queue clear â€” nice work.';
  if (input.sessionDecisions >= 50) return 'Deep focus mode â€” you are flying.';
  if (input.sessionDecisions >= 10) return 'Strong session â€” keep the rhythm.';
  if (input.dayDecisions >= 25) return 'Solid day â€” inbox is shrinking.';
  if (input.projectPercent >= 75) return 'Final stretch on this pilot batch.';
  if (input.projectPercent >= 50) return 'Past halfway â€” momentum is building.';
  if (input.projectPercent >= 25) return 'Good progress â€” every file counts.';
  if (input.sessionDecisions > 0) return 'Session started â€” one file at a time.';
