...
import { dabosDivisionHref } from '@/lib/dabos/dabos-paths';
import { deptRoleLabel } from '@/lib/dabos/org-board-config';
import { dabosConfigured, fetchDepartmentDashboardById } from '@/lib/dabos/server-data';

type PageProps = { params: Promise<{ deptId: string }> };

export default async function DepartmentPage({ params }: PageProps) {
  if (!dabosConfigured()) notFound();

  const { deptId } = await params;
  const data = await fetchDepartmentDashboardById(deptId);
  if (!data) notFound();

  const { division, department, siblings, browse } = data;
  const divisionId = department.division_id as string;
  const dept = {
  ...
  return (
    <DrilldownShell
      backHref={dabosDivisionHref(divisionId)}
