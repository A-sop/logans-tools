...
import { BookmarkPreviewPane } from '@/components/dabos-ops/dil/bookmark-preview-pane';
import { Button } from '@/components/ui/button';
import {
  linkCheckUnreliable,
  linkStatusLabel,
  linkStatusTone,
} from '@/lib/dabos-ops/contact-network/bookmark-link-check';
import { isCustomBookmarkNote, BOOKMARK_REVIEW_BATCH } from '@/lib/dabos-ops/contact-network/bookmark-review-constants';
import {
  fetchBookmarkQueue,
  fetchBookmarkStats,
  recheckBookmarkLink,
...
} from '@/lib/dabos-ops/contact-network/bookmark-review-api-client';
import type {
  BookmarkQueueItem,
...
} from '@/lib/dabos-ops/contact-network/bookmark-triage-heuristics';
  ...
  async function persistNotes(id: number, value: string): Promise<boolean> {
    ...
    try {
      ...
      if (filter === 'todo' && wasCustom) {
        ...
        setStatusMessage('Notes saved â€” find it under Noted until you Keep or Delete.');
      }
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground">
              Oldest first Â· Y keep Â· N delete Â· Notes â†’ Noted tab
            </p>
          </div>
            ...
            {UNDECIDED_FILTERS.includes(filter) ? (
              <>
                ...
                <Button
                  ...
                  title="Skip this batch without deciding â€” leaves items in the queue"
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          ...
          {queueTotal > 0 ? (
            <p className="ml-auto text-[10px] text-muted-foreground">
              Showing {batchOffset + 1}â€“{batchOffset + queueShowing} of {queueTotal}
            </p>
          ) : null}
        </div>
      </header>
          ...
          <div className="shrink-0 border-b-2 border-primary/30 bg-primary/5 px-4 py-3">
            <div className="flex items-center justify-between gap-2">
              <label htmlFor="bookmark-notes" className="text-sm font-semibold">
                Notes â€” press out the value (DABOS / GFP / wiki)
              </label>
              <span className="text-[10px] text-muted-foreground">
                {notesSaveState === 'saving'
                  ...
                  : notesSaveState === 'saved'
                    ? 'Saved'
                    : 'Autosave on blur Â· Enter = next item'}
              </span>
            </div>
            <textarea
              id="bookmark-notes"
              ...
              rows={3}
              placeholder="e.g. potential GFP content â€” then Keep or Delete when ready"
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {selectedIndex >= 0 ? (
                <p className="w-full text-center text-[10px] text-muted-foreground sm:w-auto sm:text-right">
                  {selectedIndex + 1} of {queue.length} Â· â†‘/â†“ j/k Â· Y/N
                </p>
              ) : null}
            </div>
          </div>
                ...
                <li key={item.id}>
                  ...
                  >
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1 text-[10px] text-muted-foreground">
                      ...
                      {item.bookmarkQuarter ? <span>Â· {item.bookmarkQuarter}</span> : null}
                      {item.linkStatus ? (
            ...
            <div className="space-y-4 text-sm">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  ...
                  {selected.bookmarkQuarter ? ` Â· ${selected.bookmarkQuarter}` : ''}
                </p>
                {selected.listName ? (
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {checkingLink ? 'Checkingâ€¦' : linkStatusLabel(selected.linkStatus)}
                </span>
                <Button
                  size="sm"
                  variant="outline"
              ...
              {linkCheckUnreliable(selected.linkStatus) || isFetchUnreliableHost(selected.url) ? (
                <p className="rounded-md border border-border bg-muted/30 px-3 py-2 text-[11px] text-muted-foreground">
                  Automated fetch is unreliable for this host â€” trust <strong>Open in browser</strong>, not
