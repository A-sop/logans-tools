...
import { PathAutocompleteInput } from '@/components/dabos-ops/dil/path-autocomplete-input';
...
import {
  ...
  filePreviewUrl,
  saveReviewDecision,
} from '@/lib/dabos-ops/document-intake/review-api-client';
import { mergeFilingOverrides, resolveEffectiveFiling } from '@/lib/dabos-ops/document-intake/review-filing';
import {
...
} from '@/lib/dabos-ops/document-intake/review-session-tracker';
import type { ReviewApproval, ReviewQueueFilter, ReviewQueueItem, ReviewStats } from '@/lib/dabos-ops/document-intake/review-types';
...
export function ReviewWorkbench({ className }: { className?: string } = {}) {
  ...
  /** Instant: registry read only â€” never OCR. */
  ...
  const runClassifyBatch = useCallback(async () => {
    setProcessing(true);
    setError(null);
    setStatusMessage(`Classifying up to ${REVIEW_BATCH} new inbox files (OCR â€” may take a few minutes)â€¦`);
    try {
      ...
      try {
        ...
        if (BATCHED_FILTERS.includes(filter) && advance) {
          ...
          if (remaining.length === 0) {
            if (nextStats.todo > 0) {
              setStatusMessage(`Batch done â€” loading next ${REVIEW_BATCH} from backlog (${nextStats.todo} left).`);
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground">
              Batch {REVIEW_BATCH} Â· Reload = registry only Â· Classify = OCR new inbox files
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            >
              {processing ? 'Classifyingâ€¦' : `Classify ${REVIEW_BATCH}`}
            </Button>
            <Button
              size="sm"
              variant="default"
              ...
              title="Runs dil:apply â€” dry-run preview first, then confirm to move/rename approved files for real"
            >
              {applying ? 'Applyingâ€¦' : 'Apply approved â–¶'}
            </Button>
          </div>
        </div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          ...
          {stats && motivation ? (
            <p className="ml-auto text-[10px] text-muted-foreground">
              Session {motivation.sessionDecisions} ({motivation.sessionPercent}%) Â· Today {motivation.dayDecisions} Â·
                ...
                <span className="text-[10px] text-muted-foreground">
                  {notesSaveState === 'saving'
                    ...
                    : notesSaveState === 'saved'
                      ? 'Saved'
                      : 'Autosaves on blur Â· Enter = next Â· Ctrl+Enter from notes'}
                </span>
              </div>
              <textarea
                id="review-notes"
                ...
                rows={4}
                placeholder={'Why yes/no/later. Shortcuts: "don\'t change", "archive", name: â€¦, path: â€¦'}
          ...
          <div className="shrink-0 border-b border-border bg-muted/25 px-4 py-2">
            <div className="mx-auto flex w-full max-w-5xl flex-wrap items-center justify-between gap-2">
              ...
              <p className="w-full text-center text-[10px] text-muted-foreground sm:w-auto sm:text-right">
                {selectedIndex + 1} of {queue.length} Â· â†‘/â†“ j/k Â· Y/N/L
              </p>
            </div>
          </div>
        </>
        ...
        <aside className="min-h-0 overflow-y-auto border-b border-border lg:border-b-0 lg:border-r">
          {filter === 'todo' && queueTotal > 0 ? (
            <p className="border-b border-border px-3 py-2 text-[11px] text-muted-foreground">
              Batch {Math.floor(batchOffset / REVIEW_BATCH) + 1}: {queueShowing} loaded Â· {queueTotal} in backlog Â· next batch auto-loads when this one is decided
            </p>
          ) : null}
          {loading ? (
            <p className="p-4 text-sm text-muted-foreground">Loading queueâ€¦</p>
          ) : queue.length === 0 ? (
          ...
          {selected ? (
            <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <dd className="break-all font-mono text-[11px] text-muted-foreground">
                      ...
                      {selected.registryProposedBasename === selected.currentFilename ? (
                        <span className="ml-1 text-[10px]">â€” unchanged</span>
                      ) : (
                        <span className="ml-1 text-[10px]">â€” prior machine pass</span>
                      )}
                    </dd>
                  </div>
                  <div>
                    ...
                    <dd className="break-all font-mono text-[11px] font-medium text-foreground">
                      ...
                      {selected.v3AppliedRename ? (
                        ...
                        <span className="ml-1 text-[10px] text-muted-foreground">â€” same as registry</span>
                      ) : v3Extraction ? (
                        <span className="ml-1 text-[10px] text-amber-700 dark:text-amber-400">
                          â€” piloted, needs human ({v3Extraction.chars} chars OCR)
                        </span>
                      ) : (
                        <span className="ml-1 text-[10px] text-amber-700 dark:text-amber-400">â€” not in pilot batch yet</span>
                      )}
                    </dd>
                  </div>
                  <div>
              ...
              <dl className="space-y-2 text-xs">
                ...
                <div>
                  ...
                  <dd className="font-medium">
                    {Math.round(selected.confidence * 100)}% Â· naming {Math.round(selected.namingConfidence * 100)}%
                  </dd>
                </div>
              </dl>
              ...
              <div className="rounded-md border border-border/80 bg-muted/20 p-3">
                ...
                {v3Loading ? (
                  <p className="mt-1 text-xs text-muted-foreground">Loading pilot evidenceâ€¦</p>
                ) : v3Extraction ? (
                  <div className="mt-2 space-y-2 text-xs">
                    ...
                    {v3Extraction.proposal ? (
                      ...
                      <p className="text-amber-700 dark:text-amber-400">Needs human â€” no fully-validated proposal.</p>
                    )}
                    <p className="text-muted-foreground">
                      {v3Extraction.method} Â· {v3Extraction.chars} chars
                      {v3Extraction.runAt ? ` Â· run ${v3Extraction.runAt}` : null}
                    </p>
                            ...
                            <dd className="text-muted-foreground">
                              ...
                              {field.evidence ? (
                                <span className="block font-mono text-[10px] text-muted-foreground/90">
                                  â€œ{field.evidence.slice(0, 120)}
                                  {field.evidence.length > 120 ? 'â€¦' : ''}â€
                                </span>
                              ) : null}
                            </dd>
                          </div>
              ...
              {selected.hasLearnedRule ? (
                <p className="rounded-md bg-emerald-500/10 px-2 py-1 text-xs text-emerald-800 dark:text-emerald-400">
                  ...
                  {selected.learnedRuleLabel ? (
                    <>
                      {' '}
                      â€” <span className="font-medium">{selected.learnedRuleLabel}</span>
                    </>
                  ) : null}
                ...
                {filingPreview ? (
                  <div className="rounded-md bg-muted/50 p-2 text-[11px]">
                    <div className="font-semibold">
                      ...
                      {selected.hasDraftIntent ? (
                        <span className="ml-1 font-normal text-muted-foreground">â€” draft intent from notes</span>
                      ) : null}
                    </div>
