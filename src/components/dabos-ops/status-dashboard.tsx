...
import type {
  ...
  LinearIssue,
  StatusTone,
} from '@/lib/dabos-ops/status-dashboard';
    ...
    <ul className="space-y-2">
      {issues.map((issue) => (
        <li key={issue.identifier}>
          ...
          <span className="text-muted-foreground"> â€” {issue.title}</span>
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10">
      <header className="space-y-2">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Local system snapshot â€” backup, document pipelines, and Linear at a glance.{' '}
          ...
          footer={
            !data.linear.configured ? (
              <p className="text-muted-foreground">
                Connect Linear â€” add <code>LINEAR_API_KEY</code> to <code>.env.local</code>.
              </p>
            ) : undefined
          }
        >
