...
import { getPipelinePaths } from '@/lib/atlas-ops/document-intake/config';
import { SqliteDocumentIndex } from '@/lib/atlas-ops/document-intake/index-sqlite';
import {
  loadPartnerCodeAliasMap,
  partnerCodeAliasDocPath,
} from '@/lib/atlas-ops/document-intake/partner-code-aliases';
import type { CandidateRuleRecord, LearningRuleType } from '@/lib/atlas-ops/document-intake/types';
...
async function importPartnerCodes(): Promise<void> {
  ...
  try {
    ...
    for (const alias of map.values()) {
      console.log(`  · ${alias.code} => ${alias.issuer} / ${alias.docType}`);
    }
