...
import { processRegistryFile, walkInboxFiles } from '@/lib/atlas-ops/document-intake/process-registry-file';
import { processUnprocessedInboxBatch } from '@/lib/atlas-ops/document-intake/process-inbox-batch';
import { DIL_REVIEW_BATCH_LIMIT } from '@/lib/atlas-ops/document-intake/review-config';
import { getRegistryPaths, pathHasExcludedSegment, resolveSourceRoot } from '@/lib/atlas-ops/document-intake/registry-config';
...
async function main(): Promise<void> {
  ...
  // 10_WORK, 20_ADMIN, OneDrive-DVAG). Absent → today's DATA inbox behavior, unchanged.
  ...
  if (forceUnprocessedOnly) {
    ...
    try {
      ...
      console.log('Document Intelligence — unprocessed inbox batch');
  ...
  console.log('Document Intelligence — process DATA inbox');
  console.log(`- inbox: ${paths.inboxRoot}`);
