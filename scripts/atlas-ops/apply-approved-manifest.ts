...
import { getRegistryPaths, isFrozenDestination, pathHasExcludedSegment } from '@/lib/atlas-ops/document-intake/registry-config';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // via per-file apply — the subtree-level decision wins.
    ...
    for (const row of approved) {
      if (pathHasExcludedSegment(row.source_path)) {
        console.log(`SKIP (§4 exclude): ${row.source_path}`);
        continue;
      }
      ...
      // Guard 1 (2026-07-12): stale registry rows — file already moved by a
      ...
      // the riff still needs interpretation — do not act on it here.
