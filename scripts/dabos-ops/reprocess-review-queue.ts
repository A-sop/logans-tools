...
import { processRegistryFile } from '@/lib/dabos-ops/document-intake/process-registry-file';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('DIL reprocess â€” registry review queue');
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (i % 25 === 0) {
        console.log(`â€¦ ${i}/${rows.length} (${path.basename(sourcePath)})`);
      }
      try {
    ...
    for (const [basename, count] of topDupes) {
      console.log(`  Â· ${count}x ${basename}`);
    }
  } finally {
    index.close();
  }
